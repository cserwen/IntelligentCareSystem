...
public interface EmployeeMapper {
    ...
    @Insert("insert into employee_info(ORG_ID, CLIENT_ID, username, password, gender,phone,id_card,birthday,hire_date, resign_date, imgset_dir," +
            ...
            "values(1,1,#{username}, #{password}, #{gender},#{phone},#{idCard},#{birthday}, #{hireDate}, #{resignDate}, #{imgSetDir}," +
            " #{profilePhotoDir}, #{description}, #{isActive},#{createTime}, #{createBy}, #{updateTime}," +
            " #{updateBy},#{remove})")
    void add(Employee employee);

    @Update("update employee_info set username=#{username}, password=#{password}, gender=#{gender}, phone=#{phone}, id_card=#{idCard}," +
